...
module.exports = class Page21 extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    return(
      <View>
        <Text>这是交易的第2-1个页面</Text>
        <Text>{`交易数据: ${TradeUtil.getTradeData("tradedata")}`}</Text>
        <Text>点击按钮设置数据，在下一步可以看到</Text>
        <Button title={`设置数据`} onPress={() => {
          TradeUtil.setTradeData("step", "2.1");
        }}></Button>
        <Text>点击按钮进入下一步</Text>
